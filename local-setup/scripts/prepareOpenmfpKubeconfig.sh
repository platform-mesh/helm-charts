...
KCP_ROOT_URL=https://openmfp-kcp-front-proxy.openmfp-system:8443/clusters/root
...
# Retrieve Virtual Workspace / Endpoint URL
OPENMFP_SYSTEM_KUBECONFIG=$(kubeconfig ${KCP_OPENMFP_SYSTEM_WS_URL})
ROOT_KUBECONFIG=$(kubeconfig ${KCP_ROOT_URL})

OPENMFP_OPERATOR_KUBECONFIG=$(kubeconfig "https://openmfp-kcp-front-proxy.openmfp-system:8443")

kubectl apply --context=${CONTEXT} -f - <<EOF
apiVersion: v1
data:
  kubeconfig: $(echo "${OPENMFP_OPERATOR_KUBECONFIG}" | base64 -w 0)
kind: Secret
metadata:
