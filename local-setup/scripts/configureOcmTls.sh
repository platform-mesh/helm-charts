...
if [ ! -f "$SCRIPT_DIR/certs/ca.crt" ]; then
  echo "Error: $SCRIPT_DIR/certs/ca.crt not found"
  exit 1
fi
...
kubectl -n ocm-system create configmap ocm-custom-ca --from-file=registry-ca.pem=$SCRIPT_DIR/certs/ca.crt --dry-run=client -oyaml | kubectl apply -f -
