...
RUNTIME_KUBECOFIG=.secret/platform-mesh.kubeconfig

mkdir -p $PWD/.secret/kcp
kubectl --kubeconfig=$RUNTIME_KUBECOFIG --context kind-platform-mesh get secret kubeconfig-kcp-admin -n platform-mesh-system -o=jsonpath='{.data.kubeconfig}'|base64 -d > $PWD/.secret/kcp/admin.kubeconfig
