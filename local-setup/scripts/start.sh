...
if [ "$EXAMPLE_DATA" = true ]; then
  ...
  kubectl create-workspace providers --type=root:providers --ignore-existing --server="https://kcp.api.localhost:8443/clusters/root"
  kubectl create-workspace httpbin-provider --type=root:provider --ignore-existing --server="https://kcp.api.localhost:8443/clusters/root:providers"
  kubectl apply -k $SCRIPT_DIR/../example-data/root/providers/httpbin-provider --server="https://kcp.api.localhost:8443/clusters/root:providers:httpbin-provider"
  unset KUBECONFIG
