...
const ORG_SHARD: ShardName =
  (process.env.SHARDING_ORG_SHARD as ShardName) || "triton";
...
async function completeKeycloakSetup(page: Page): Promise<void> {
  ...
  await page.waitForURL(`https://${orgName}.portal.localhost:8443/**`, {
    timeout: 30000,
  });
  await expect(
    page.getByText("Welcome! Let's get started.", { exact: true }),
...
test.describe("Home Page", () => {
  ...
  test("Register and navigate to portal", async ({ page }) => {
    ...
    await page
      .getByRole("textbox", { name: "Password" })
      .fill(primaryUser.keycloakPassword);
    await page.getByRole("button", { name: "Sign In" }).click();
