...
async function openAccountsView(page: Page, orgName?: string): Promise<void> {
  ...
  await Promise.race([
    ...
    heading
      ...
      .then(() => createButton.isVisible().catch(() => false)),
  ]).catch(() => false);
...
async function ensureAccountExists(
  ...
  if (!(await accountRow.isVisible().catch(() => false))) {
    ...
    const typeSelectInfo = await page
      .evaluate(() => {
        function findRequiredEmptySelect(
    ...
    const alreadyExistsAlert = page.getByText(
      `accounts.core.platform-mesh.io "${account}" already exists`,
    );
    const transientWebhookAlert = page.getByText(
      /failed calling webhook|connection refused|Internal error occurred/i,
    );
    ...
    for (let attempt = 0; attempt < 4; attempt++) {
      ...
      if (attempt === 3) {
        throw new Error(
          "Account creation kept failing due to transient webhook errors",
        );
      }

      const alertCloseButton = page
  ...
  logStep(
    `ensureAccountExists:navigate-direct account=${account} url=${accountUrl}`,
  );
  await page.goto(accountUrl, { waitUntil: "domcontentloaded" });
  await expect(
    page.getByRole("button", { name: "Download kubeconfig" }),
  ).toBeVisible({ timeout: 30000 });
  logStep(`ensureAccountExists:done account=${account}`);
  return accountUrl;
}

async function downloadAccountKubeconfig(
  page: Page,
  orgName?: string,
  accountName?: string,
): Promise<string> {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    const closeButton = page.locator(
      'ui5-message-strip-alert button, [ref="e6"]',
    );
...
async function deleteAccount(
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    if (attempt === 2) {
      throw new Error(
        `Account dashboard did not become ready before deletion, final URL=${page.url()}`,
      );
    }
  ...
  if (!clickedDelete) {
    ...
    throw new Error(
      `Could not find account delete button on the account detail page, buttons=${JSON.stringify(visibleButtons)}`,
    );
  }

  // Wait for the delete confirmation dialog to appear using data-testid
  const deleteDialog = page.locator('[data-testid="delete-resource-dialog"]');
  await expect(deleteDialog).toBeVisible({ timeout: 10000 });

  // Find and fill the confirmation input inside the dialog using data-testid
