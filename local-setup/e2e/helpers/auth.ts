...
async function loginToInitialKeycloak(
  page: Page,
  user: TestUser,
): Promise<void> {
  await page.getByRole("textbox", { name: "Email" }).fill(user.email);
...
async function settleKeycloakFlow(
  page: Page,
  user: TestUser,
  orgPortalUrl: string,
): Promise<void> {
  ...
  const backToApplication = page.getByRole("link", {
  ...
  if (
    !page.url().startsWith(orgPortalUrl) &&
    !page.url().includes("/keycloak/")
  ) {
...
async function fillOrganizationField(page: Page, value: string): Promise<void> {
  ...
  const typedValue = await host.evaluate(
...
async function selectExistingOrganization(
  page: Page,
  value: string,
): Promise<boolean> {
  ...
  if (optionIndex === -1) {
    logStep(
      `selectExistingOrganization:not-found value=${value} options=${optionTexts.join(",") || "none"}`,
    );
...
async function waitForOrganizationSwitchReady(page: Page): Promise<void> {
  const switchButton = page
    .locator('[data-testid="organization-management-switch-button"]')
    .locator("button");
  ...
  await expect
    .poll(
      ...
      { timeout: Number(orgReadyTimeoutSeconds) * 1000 },
    )
    .toMatchObject({
      enabled: true,
    });
...
async function ensureWelcomePage(page: Page, user: TestUser): Promise<void> {
  ...
  await page
    .waitForURL(`${portalBaseUrl}**`, { timeout: 10000 })
    .catch(() => {});
  await page.waitForLoadState("load", { timeout: 10000 }).catch(() => {});

  const landingState = await Promise.race([
    page
      .getByText("Welcome to the Platform Mesh Portal!")
      .waitFor({ state: "visible", timeout: 10000 })
      .then(() => "welcome"),
    page
      .locator('[data-testid="organization-management-input"]')
      .waitFor({ state: "visible", timeout: 10000 })
      .then(() => "org-management"),
    page
      .getByText("Welcome! Let's get started.", { exact: true })
      .waitFor({ state: "visible", timeout: 10000 })
      .then(() => "home"),
  ]).catch(() => "none");
  ...
  if (landingState === "none") {
    throw new Error(
      `Portal welcome page did not reach a usable post-login state, final URL=${page.url()}`,
    );
  }
}

async function ensurePortalHome(
  page: Page,
  user?: TestUser,
  orgName?: string,
): Promise<void> {
  const org = orgName ?? newOrgName;
  const welcome = page.getByText("Welcome! Let's get started.", {
    exact: true,
  });
...
async function switchToOrganization(
  ...
  const orgPortalBaseUrl = `https://${org}.portal.localhost:8443/`;
  const existingOrgAlert = page
    .getByText(
      new RegExp(
        `organization.*${org}.*already exists|${org}.*already exists`,
        "i",
      ),
    )
    .first();
  ...
  if (createIfMissing) {
    const onboardButton = page
      .locator('[data-testid="organization-management-onboard-button"]')
      .locator("button");
  ...
  if (!page.url().startsWith(orgPortalBaseUrl)) {
    throw new Error(
      `Organization switch did not land on ${orgPortalBaseUrl}, final URL=${page.url()}`,
    );
  }
...
async function inviteUserToOrg(
  ...
  const addButton = membersFrame.locator(
  ...
  const addMembersFrame = page.frameLocator(
    'iframe[src*="organization/add-members"]',
  );
  const userInput = addMembersFrame
    .locator('[data-testid="app-iam-member-add-dialog-user-search-input"]')
    .locator("input");
  ...
  const inviteInFrame = addMembersFrame
    .locator("li.fd-combobox-list-item")
    .filter({ hasText: "Invite" })
    .filter({ hasText: userEmailToInvite });
  const inviteInPage = page
    .locator("li.fd-combobox-list-item")
    .filter({ hasText: "Invite" })
    .filter({ hasText: userEmailToInvite });
  ...
  await expect(
    addMembersFrame.getByRole("row").filter({ hasText: userEmailToInvite }),
  ).toBeVisible({ timeout: 15000 });
  await addMembersFrame
    .locator('[data-testid="app-iam-member-add-dialog-add-button"]')
    .click();

  const memberEmailInList = membersFrame
    .locator("span.member-extra-information")
    .filter({ hasText: userEmailToInvite });
