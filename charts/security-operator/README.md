...
| initializer.idpSeed.enabled | bool | `false` | Enable upstream identity provider seeding for local development |
| initializer.idpSeed.seedUpstreamIdentityProviders.providers | list | `[]` |  |
| initializer.idpSeed.seedUpstreamIdentityProviders.realms | list | `[]` | Org/realm names that receive seeded upstream providers (empty = none) |
